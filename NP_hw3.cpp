		...
		case WM_SOCKET_NOTIFY:
			switch( WSAGETSELECTEVENT(lParam) )
			{
						...
						for (int i = 0; i < MaxHost; i++){
							...
							if (strcmp(ip[i], "") != 0 && strcmp(port[i], "") != 0 && strcmp(file[i], "") != 0){
								...
								if (fp[i] != NULL){
									...
									err = WSAAsyncSelect(csock[i], hwnd, WM_SOCKET_NOTIFY, FD_READ | FD_CLOSE);
				...
				//Write your code for read event here.
					...
					if(len > 0 || ( len==SOCKET_ERROR && WSAGetLastError()==WSAEWOULDBLOCK )){
						...
						if(strchr(ResultFromServer,'%')!=NULL){
							err = WSAAsyncSelect(csock[index], hwnd, WM_SOCKET_NOTIFY, FD_WRITE | FD_CLOSE);
						}
					}
					else{
				...
				//Write your code for write event here
					...
					//int len;
					//char command[Maxlinelen];
					len = readfile(fp[index], command, Maxlinelen);
					if (len > 0){
					...
					err = WSAAsyncSelect(csock[index], hwnd, WM_SOCKET_NOTIFY, FD_READ | FD_CLOSE);
					break;
				case FD_CLOSE:
					if (conn == 0){
						send(ssock, "</font>\n", strlen("</font>\n"), 0);
